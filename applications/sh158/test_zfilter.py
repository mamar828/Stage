...
gradient = zfilter(test_array_gaussian)
...
figs[1].add_elements(gl.Heatmap(test_array_gaussian - rand, show_color_bar=True))
figs[2].add_elements(gl.Heatmap(test_array_gaussian, show_color_bar=True))
figs[3].add_elements(gl.Heatmap(test_array_gaussian - gradient, show_color_bar=True, vmin=-1.5, vmax=1.5))
figs[4].add_elements(gl.Heatmap(gradient, show_color_bar=True))
figs[5].add_elements(gl.Heatmap(rand - (test_array_gaussian-gradient), show_color_bar=True, vmin=-1.5, vmax=1.5))

fig = gl.MultiFigure.from_grid(figs, (2,3), size=(14,9))
fig.save("figures/zfilter/zurflueh_test_gaussian.pdf")
